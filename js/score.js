const Score = this.Score = function () {
  const score = this;
  ...
  let _randomNumberGeneratorSeed;

  let _updatePartDebounced;
  ...
  function reseed(seed) {
    _randomNumberGeneratorSeed = seed || Math.round(Math.random() * 1000);
    score.trigger('change');
  }
  ...
  function insertNote(newNote) {
    ...
    const newNotes = [];
    _notes.forEach((note) => {
      const transformedNoteList = noteAfterRangeInsertion(note, newNote);
      transformedNoteList.forEach((transformed) => {
